...
const db = mysql.createConnection({
  ...
  password: 'RRH6fCmDja2D0NWKbMI1', // Add your MySQL password here
...
app.get('/api/users', verifyToken, (req, res) => {
    ...
    SELECT u.id, u.name, u.email, u.role, u.created_at, u.last_login,
           GROUP_CONCAT(
             CASE 
               ...
               THEN JSON_OBJECT('unit_number', un.unit_number, 'unit_name', un.unit_name)
               ELSE NULL 
             END
           ) as units
    FROM users u
    LEFT JOIN units un ON u.id = un.userId
    GROUP BY u.id`;
  ...
  db.query(query, (err, results) => {
    ...
    const users = results.map(user => ({
      ...user,
      units: user.units ? JSON.parse(`[${user.units}]`) : []
    }));
...
app.post('/api/users', verifyToken, async (req, res) => {
  ...
  // Validate single unit for user role
  if (role === 'user') {
    if (!units || units.length === 0) {
      return res.status(400).json({ message: 'A unit must be assigned to the user' });
    }
    if (units.length > 1) {
      return res.status(400).json({ message: 'Only one unit can be assigned per user' });
    }
    if (!units[0].unit_number || !units[0].unit_name) {
      return res.status(400).json({ message: 'Unit number and name are required' });
    }
  }
  ...
  try {
    ...
    db.query(checkQuery, [email], async (err, results) => {
      ...
      db.query(insertQuery, [name, email, hashedPassword, role || 'user'], async (err, result) => {
        ...
        // If role is user, assign the single unit
        if (role === 'user' && units && units.length === 1) {
          const unit = units[0];
          const insertUnitQuery = 'INSERT INTO units (unit_number, unit_name, userId) VALUES (?, ?, ?)';
          
          db.query(insertUnitQuery, [unit.unit_number, unit.unit_name, userId], (err) => {
            if (err) {
              console.error('Error assigning unit:', err);
              return res.status(500).json({ message: 'User created but failed to assign unit' });
            }
            
            res.status(201).json({ 
              message: 'User created successfully with unit assigned',
              userId: userId
            });
          });
        } else {
...
app.put('/api/users/:id', verifyToken, async (req, res) => {
  ...
  try {
    ...
    // Handle units if role is 'user'
    if (role === 'user') {
      // Delete existing units
      await new Promise((resolve, reject) => {
        db.query('DELETE FROM units WHERE userId = ?', [userId], (err) => {
          if (err) reject(err);
          else resolve();
        });
      });
      ...
      // Add new units
      if (units && units.length > 0) {
        const unitValues = units.map(unit => [userId, unit.unit_number, unit.unit_name]);
        await new Promise((resolve, reject) => {
          db.query(
            'INSERT INTO units (userId, unit_number, unit_name) VALUES ?',
            [unitValues],
            (err) => {
              if (err) reject(err);
              else resolve();
...
app.delete('/api/users/:id', verifyToken, async (req, res) => {
  ...
  try {
    ...
    // Delete user's units first (foreign key constraint)
    await new Promise((resolve, reject) => {
      db.query('DELETE FROM units WHERE userId = ?', [userId], (err) => {
        if (err) reject(err);
        else resolve();
  ...
  db.query(countQuery, (err, countResult) => {
    ...
    db.query(query, [limit, offset], (err, results) => {
      ...
      const formattedResults = results.map(entry => ({
        ...entry,
        recordedBy: entry.units 
          ? `${entry.recordedBy} (${entry.units})`
          : entry.recordedBy
      }));
...
app.get('/api/profile', verifyToken, (req, res) => {
  ...
  const query = 'SELECT id, name, email, role, created_at, last_login FROM users WHERE id = ?';
...
app.get('/api/driver-info/:mobile', verifyToken, (req, res) => {
  ...
  db.query(query, [mobile], (err, results) => {
    ...
    if (results.length === 0) {
      return res.status(404).json({ message: 'No driver found with this mobile number' });
    }

    // Return all drivers found with this mobile number
    res.json({ 
      drivers: results.map(row => ({ driverName: row.driverName }))
    });
  });
});
